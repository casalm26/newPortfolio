...
describe('ProjectNavigation Component', () => {
  ...
  describe('Visual states', () => {
    it('should apply different styles for available vs unavailable navigation', () => {
      ...
      const prevContainer = screen.getByText('PREV').closest('div')?.parentElement;
      const nextContainer = screen.getByText('NEXT').closest('div')?.parentElement;
      
      // Previous should have disabled styling (not a link)
      expect(prevContainer).toHaveClass('border-terminal-600', 'text-terminal-600');
      
      // Next should have enabled styling (is a link)
      expect(nextContainer).toHaveClass('border-terminal-400', 'text-terminal-300');
    });
  ...
  describe('Edge cases', () => {
    it('should return null for invalid project slug', () => {
      const { container } = render(<ProjectNavigation currentSlug="invalid-project" />);
    ...
    it('should handle empty project list gracefully', () => {
      ...
      const { container } = render(<ProjectNavigation currentSlug="any-project" />);
      expect(container.firstChild).toBeNull();
    });
