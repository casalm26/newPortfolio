...
vi.mock('@/components/landing', () => ({
  LandingProductFeaturesGrid: ({ children, title, description, className }: {
  ...
  LandingProductFeature: ({ title, description, leadingComponent }: {
    title: string;
    description: string;
...
vi.mock('lucide-react', () => ({
  ...
  BarChart4: ({ className }: { className?: string }) => <div data-testid="chart-icon" className={className} />,
  Globe: ({ className }: { className?: string }) => <div data-testid="globe-icon" className={className} />,
  Rocket: ({ className }: { className?: string }) => <div data-testid="rocket-icon" className={className} />,
  Settings: ({ className }: { className?: string }) => <div data-testid="settings-icon" className={className} />,
  Headphones: ({ className }: { className?: string }) => <div data-testid="headphones-icon" className={className} />
}));
...
describe('Services Page', () => {
  describe('Page Structure', () => {
    ...
    it('should render the page title and description', () => {
      ...
      expect(screen.getByText(/We offer comprehensive solutions/)).toBeInTheDocument();
    });
    ...
    it('should have the correct page structure classes', () => {
      ...
      expect(mainContainer).toHaveClass('flex', 'flex-col', 'w-full', 'min-h-screen');
    });
  });
  ...
  describe('Services Grid', () => {
    it('should render the features grid with correct props', () => {
      ...
      expect(screen.getByText('Complete Solutions for Your Business')).toBeInTheDocument();
      expect(screen.getByText('From strategy to implementation, we provide end-to-end services that drive results.')).toBeInTheDocument();
    });
  ...
  describe('Why Choose Our Services Section', () => {
    ...
    it('should render value proposition descriptions', () => {
      ...
      expect(screen.getByText(/With hundreds of successful projects/)).toBeInTheDocument();
      expect(screen.getByText(/We understand that every business is unique/)).toBeInTheDocument();
      expect(screen.getByText(/Our team consists of industry experts/)).toBeInTheDocument();
      expect(screen.getByText(/We don't just deliver and leave/)).toBeInTheDocument();
    });
  });
  ...
  describe('Content Accuracy', () => {
    it('should have correct service descriptions', () => {
      ...
      expect(screen.getByText(/Expert guidance to develop winning strategies/)).toBeInTheDocument();
      expect(screen.getByText(/Custom software development and technology implementation/)).toBeInTheDocument();
      expect(screen.getByText(/Comprehensive security solutions and compliance management/)).toBeInTheDocument();
      expect(screen.getByText(/Professional training programs to upskill your team/)).toBeInTheDocument();
    });
  });
  ...
  describe('Responsive Design', () => {
    it('should have responsive classes on grid sections', () => {
      ...
      const whyChooseGrid = screen.getByText('Proven Track Record').closest('.grid');
