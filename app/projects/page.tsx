    ...
    <>
      {parts.map((part, index) =>
        part.toLowerCase() === searchTerm.toLowerCase() ? (
          <span key={index} className="bg-yellow-400 text-black px-1 font-semibold">
            {part}
          </span>
        ) : (
          part
    ...
    >
      <div className="flex flex-col space-y-3">
        ...
        <div className="flex items-center justify-between pt-2">
          <span className="font-pixel text-xs text-terminal-400">
            <HighlightText text={project.role} searchTerm={searchTerm} />
          </span>
  ...
  const sortedProjects = useMemo(() => {
    ...
    switch (sortBy) {
      case "name":
        ...
        break;
      case "type":
        sorted = sorted.sort((a, b) => a.projectType.localeCompare(b.projectType));
        break;
      case "relevance":
        // Sort by skills match if searching
        if (searchTerm) {
          sorted = sorted.sort((a, b) => {
            const aMatches = (a.skills?.join(" ") + " " + a.summary).toLowerCase().includes(searchTerm.toLowerCase()) ? 1 : 0;
            const bMatches = (b.skills?.join(" ") + " " + b.summary).toLowerCase().includes(searchTerm.toLowerCase()) ? 1 : 0;
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (techFilter) {
      filtered = filtered.filter((project) =>
        project.skills?.some(skill => skill.toLowerCase().includes(techFilter.toLowerCase())) ||
        project.tools?.some(tool => tool.toLowerCase().includes(techFilter.toLowerCase()))
      );
    }
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {projectTypes.map((type) => {
                const count = allProjects.filter((p) => p.projectType === type).length;
        ...
        {filteredProjects.length > 0 ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {filteredProjects.map((project) => (
              <ProjectCard key={project.slug} project={project} searchTerm={searchTerm} />
            ))}
          </div>
        ) : (
          <div className="text-center py-12">
            <div className="font-pixel text-terminal-400 mb-4">
              {searchTerm ? "No projects match your search criteria" : "No projects found with selected filters"}
            </div>
            <button
              onClick={() => {
                setSearchTerm("");
