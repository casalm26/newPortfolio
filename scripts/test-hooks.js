...
try {
  ...
  try {
    ...
    console.log('⚠️ Docker or Docker Compose not found - Docker testing will be skipped');
    console.log('ℹ️ Install Docker to enable full-stack testing capabilities');
  }

  // Test all hook files exist
  const requiredHooks = ['.husky/pre-commit', '.husky/pre-push', '.husky/commit-msg'];
  ...
  if (!fs.existsSync('database/test-schema.sql')) {
    console.log('⚠️ Test database schema not found - creating directory structure...');
    fs.mkdirSync('database', { recursive: true });
    fs.writeFileSync('database/test-schema.sql', '-- Add your test schema here');
  }
  ...
  if (dockerAvailable) {
    ...
    try {
      execSync('docker-compose -f docker-compose.test.yml config', { stdio: 'pipe' });
  ...
  if (!fs.existsSync('commitlint.config.js')) {
    console.log('⚠️ Commitlint configuration missing - conventional commits not enforced');
  } else {
  ...
  console.log('💡 Run "npm run test:docker:all" to test the complete Docker setup');
